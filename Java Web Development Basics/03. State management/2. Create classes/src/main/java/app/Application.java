...
import main.java.app.impl.HttpCookieImpl;
import main.java.app.impl.HttpRequestImpl;
import main.java.app.impl.HttpResponseImpl;
...
//        List<String> validURLs = getValidURLs();
        ...
        HttpRequestImpl request = new HttpRequestImpl(getRequestString());

//        HttpResponseImpl response = generateResponse(request, validURLs);
        ...
        if(request.getHeaders().containsKey("Cookie")){
            Arrays.stream(request.getHeaders()
                    ...
                    .map(c->c.split("="))
                    .forEach(c->{
                        HttpCookie httpCookie = new HttpCookieImpl(c[0],c[1]);
...
//        System.out.println(response);
    ...
    private static HttpResponseImpl generateResponse(HttpRequestImpl request, List<String> validURLs) {
